...
class PostViewSet(viewsets.ModelViewSet):
    ...
    #  
    @action(detail=False, methods=['GET'])
    def morelikes(self, request):
        """посты с большим количеством лайков"""
        rec_post = Post.objects.annotate(num_likes=Count('likes')).order_by('-num_likes')
    ...
    def favorite(self, request, pk=None):
        """Добавление и удаление избарнных постов"""
        post = self.get_object()
        favorite = request.user.favorites.filter(post=post).all()
        if favorite:
            favorite.delete()
            return Response('удалено из избранных', 200)
